...
export async function getUserFamilyId(uid: string): Promise<string | null> {
  ...
  return (snap.data()?.familyId as string | null) ?? null;
}

export function eventDisplayName(user: Record<string, unknown> | undefined, fallback: string): string {
...
export function assertSingleFamily(familyId: string | null): void {
  if (familyId) {
...
export function mergeUpdatedAt(data: Record<string, unknown>): Record<string, unknown> {
  return { ...data, updatedAt: SERVER_TIMESTAMP };
}

export function asTimestampOrNull(value: unknown): FirebaseFirestore.Timestamp | null {
  if (value == null) {
    return null;
  }
