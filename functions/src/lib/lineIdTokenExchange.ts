...
export type ExchangeLineIdTokenRequest = {
  idToken: string;
  channelId?: string;
};

export type ExchangeLineIdTokenResponse = {
  ok: true;
  customToken: string;
  lineUserId: string;
...
export async function exchangeLineIdTokenCore(body: ExchangeLineIdTokenRequest): Promise<ExchangeLineIdTokenResponse> {
  ...
  const verified = (await res.json()) as { sub?: string; name?: string; picture?: string; aud?: string };
  if (!verified.sub) {
